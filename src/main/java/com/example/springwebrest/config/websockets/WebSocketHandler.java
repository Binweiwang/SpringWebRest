...
public class WebSocketHandler extends TextWebSocketHandler implements SubProtocolCapable, WebSocketSender{
    ...
    public void sendMessage(String message) throws IOException {
        log.info("Enviando mensaje de cambios en la entidad: " + entity + " - " + message);
        for (WebSocketSession session : sessions) {
            log.info("Enviando mensaje a la sesión: " + session);
    ...
    public void sendPeriodicMessages() throws IOException {
        for (WebSocketSession session : sessions) {
            log.info("Enviando mensaje a la sesión: " + session);
            session.sendMessage(new TextMessage("Periodic message: " + LocalTime.now()));
        }
    }
    @Override
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        log.error("Error en la conexión: " + exception.getMessage());
    }
}
